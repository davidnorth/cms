...
/* URL for image library */
var imageLibraryURL = "/admin/images";
var documentLibraryURL = "/admin/files";

/* Location of stylesheet file for editor content */
var widgStylesheet = "/stylesheets/widgContent.css";

/* Items to appear in toolbar. */
widgToolbarItems = [ "bold", "italic", "hyperlink", "documentlink", "unorderedlist", "orderedlist", "image", "htmlsource", "blockformat", "style" ]
...
widgSelectStyleOptions = [
  '','Paragraph style',
  'highlight','Highlight box',
  'imgl','Image on left',
  'imgr','Image on right',
  'imgl_noborder','Image on left no border',
  'imgr_noborder','Image on right no border'
  ];
...
function wysiwyg(replacedTextareaID)
{
	...
	this.theIframe.style.height = (this.theTextarea.offsetHeight - 32) + 'px';
...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "documentlink":
				this.addButton(this.theList.id + "ButtonLink", "widgButtonDocumentlink", "Document Link", "documentlink");
			...
			case "style":
				this.addSelect(this.theList.id + "SelectStyle", "widgSelectStyle", widgSelectStyleOptions, "formatstyle");
				break;
		}
	}
...
widgToolbar.prototype.setState = function(theState, theStatus)
{
	...
	if (theState == "SelectBlock" || theState == "SelectStyle")
	{
	...
	switch (this.action)
	{
      ...
      theNode.className = this.value;
		...
		case "link":
			...
			break;
		case "documentlink":
      theIframeForDocumentLink = theIframe;
      window.open(documentLibraryURL,'documentLibraryWindow','width=800,height=600,scrollbars=auto');
  		break;	
			
		case "image":
  		insertImage(theIframe);
		  break;
		default:
...
function widgGetParentNode(thewysiwyg)
{
    ...
    if(theSelection.anchorNode.childNodes[0] && theSelection.anchorNode.childNodes[0].nodeName == 'IMG'){
      theParentNode = theSelection.anchorNode.childNodes[0];
    }
    else {
...
function widgInsertDocumentLink(theIframe,src,filename,filetype)
{
	/* IE selections */
	if (theIframe.contentWindow.document.selection)
	{
    ...
    html = '<a href="' + src + '" class="document_link ' + filetype + '">' + filename + '</a>';
	...
	else
	{
		...
		theLinkNode.innerHTML = filename
		theLinkNode.className = 'document_link ' + filetype
		theLinkNode.href = src
		theRange.insertNode(theLinkNode);		
	}

}

function widgInsertImage(theIframe,theImage,theAlt)
{
	/* IE selections */
	if (theIframe.contentWindow.document.selection)
	{
